...
{
	if (this->_type.compare("FR4G-TR") == 0 || this->_type.compare("SUP3R-TR") == 0)
...
void		ClapTrap::takeDamage(unsigned int amount)
{
	...
	if (this->_HP == 0)
	{
		...
		else if (this->_type.compare("SUP3R-TR") == 0)
			std::cout << "SUP3R-TR DEFEATED. IMPOSSIBLE STATISTICS : SEGFAULT (CORE DUMPED)." << std::endl;
		else
	...
	if (this->_type.compare("FR4G-TR") == 0)
		...
		std::cout << "RESISTANCE IS FUTILE. SURRENDER NOW OR DIE.\n" << this->_type << " " << this->_name << " just go hit for " << i;
	else
...
void		ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (this->_type.compare("FR4G-TR") == 0)
		...
		std::cout << "HEALING IS UNNECESSARY, BUT APPRECIATED. THANK YOU MORTAL.\n" << this->_type << " " << this->_name << " just healed for " << i << std::endl;
	else
